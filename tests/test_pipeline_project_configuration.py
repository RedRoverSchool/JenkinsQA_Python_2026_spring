...
def test_disable_pipeline(browser):
    ...
    assert warning_message.is_displayed(), "Сообщение об откл. мне отображается"


@pytest.mark.dependency(depends=["test_create_pipeline_project", "test_disable_pipeline"])
def test_enable_pipeline_with_button(browser):
    wait = WebDriverWait(browser, 5)
    ...
    browser.find_element(By.XPATH, f"//a[@href='/job/{PIPELINE_NAME}/']").click()

    wait.until(EC.element_to_be_clickable(
        (By.CSS_SELECTOR, "button[name='Submit'][value='Enable']")
    )).click()
    ...
    toggle = wait.until(
        EC.visibility_of_element_located((By.ID, "toggle-switch-enable-disable-project"))
    )
    assert "Enabled" in toggle.text, f"Проект не включился: тоггл показывает '{toggle.text}'"